...
export default class CSSComputedStyle {
    ...
    getComputedStyle() {
        ...
        const processedCustomElements = [];
        let shadowRootElements = [];
        let allShadowRootElements = [];
        ...
        while (styleAndElement.element) {
            ...
            if (styleAndElement.element === this.element[PropertySymbol.ownerDocument]) {
                ...
                if (styleAndElement.element[PropertySymbol.nodeType] === NodeTypeEnum.elementNode &&
                    styleAndElement.element.shadowRoot &&
                    !processedCustomElements.includes(styleAndElement)) {
                    const shadowRoot = styleAndElement.element.shadowRoot;
                    if (!styleAndElement.propertyManager) {
                        const styleSheets = this.getStyleSheets(shadowRoot);
                        for (const styleSheet of styleSheets) {
                            this.parseCSSRules({
                ...
                styleAndElement = {
                    ...
                    cssTexts: [],
                    propertyManager: this.getCachedPropertyManager(styleAndElement.element[PropertySymbol.parentNode])
                };
            }
        }
        ...
        for (let i = 0, max = parentElements.length; i < max; i++) {
            ...
            if (parentElement.propertyManager) {
                ...
                for (const name of Object.keys(parentElement.propertyManager.properties)) {
                    if (CSSComputedStyleInheritedProperties[name]) {
                        inheritedPropertyManager.set(name, parentElement.propertyManager.get(name).value, parentElement.propertyManager.get(name).important);
                    }
                }
                ...
                const defaultCSS = CSSComputedStyleElementDefault[parentElement.element[PropertySymbol.tagName]];
                let elementCSSText = '';
                if (defaultCSS) {
                    ...
                    if (parsedValue &&
                        ...
                        if (!settings?.disableComputedStyleRendering) {
                            ...
                            for (const key of affectedKeys) {
                                if (CSSComputedStyleMeasurementProperties[key]) {
                ...
                parentElement.element[PropertySymbol.ownerDocument][PropertySymbol.affectsComputedStyleCache].push(cachedResult);
                ...
                for (let a = 0; a < i; a++) {
                    ...
                    if (previousParent.element?.shadowRoot) {
                        previousParent.element.shadowRoot[PropertySymbol.affectsCache].push(cachedResult);
                    }
                }
                if (parentElement.element === this.element) {
...
//# sourceMappingURL=CSSComputedStyle.js.map