...
export default class QuerySelector {
    static globalMatchFunction = this.matches.bind(this);
    /**
    ...
    static querySelectorAll(node, selector) {
        ...
        const groups = new SelectorParser({ window, scope, globalMatchFunction }).getSelectorGroups(selector);
        const items = [];
        ...
        if (node[PropertySymbol.isConnected]) {
            ...
            if (node[PropertySymbol.nodeType] === NodeTypeEnum.documentFragmentNode && node.host) {
                node.host[PropertySymbol.affectsCache].push(cachedItem);
            }
        }
        for (const selectorItems of groups) {
            const rootElement = node[PropertySymbol.nodeType] === NodeTypeEnum.elementNode ||
                (node[PropertySymbol.nodeType] === NodeTypeEnum.documentFragmentNode && node.host)
                ? node
                : null;
            const children = rootElement ? [node] : node[PropertySymbol.elementArray];
    ...
    static matches(element, selector, options) {
        ...
        if (element[PropertySymbol.isConnected]) {
            ...
            if (element[PropertySymbol.nodeType] === NodeTypeEnum.documentFragmentNode &&
                element.host) {
                element.host[PropertySymbol.affectsCache].push(cachedItem);
            }
        }
            ...
            switch (selectorItem.combinator) {
                ...
                case SelectorCombinatorEnum.descendant:
                    ...
                    if (parentElement && parentElement !== element[PropertySymbol.ownerDocument]) {
                        this.affectsCache(parentElement, cachedItem);
                        const match = this.matchSelector({
                            scope,
                ...
                case SelectorCombinatorEnum.subsequentSibling:
                    ...
                    if (siblingParentElement &&
                        ...
                        this.affectsCache(siblingParentElement, cachedItem);
     ...
     */
    static affectsCache(element, cachedItem) {
